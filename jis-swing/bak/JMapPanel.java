...
public class JMapPanel extends JPanel implements
    ...
    /**
     * A factory for positions
     */
    private PositionFactory positionFactory    = null;
     ...
     */
    private Position drawnLinePt1  = null;
    private Position drawnLinePt2  = null;
          ...
          public void mouseClicked(MouseEvent e) {
            ...
            switch (JMapPanel.this.state) {
              ...
              case STATE_DRAW_LINE:
                ...
                if (e.getButton() == MouseEvent.BUTTON1){
                  ...
                  if (JMapPanel.this.drawnLinePt1 == null){
                    JMapPanel.this.drawnLinePt1 = JMapPanel.this.positionFactory.createDirectPosition(new double[]{JMapPanel.this.mapX, JMapPanel.this.mapY});
                  } else if (JMapPanel.this.drawnLinePt2 == null){
                    JMapPanel.this.drawnLinePt2 = JMapPanel.this.positionFactory.createDirectPosition(new double[]{JMapPanel.this.mapX, JMapPanel.this.mapY});
              ...
              case STATE_DRAW_POLYGON:
        	    ...
        	    pt = JMapPanel.this.positionFactory.createDirectPosition(new double[]{JMapPanel.this.mapX, JMapPanel.this.mapY});
...
//EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
    ...
    private void processDrag(int x1, int y1, int x2, int y2) {
        ...
        if (this.state == JMapPanel.STATE_PAN) {
          ...
          positions.add(this.positionFactory.createDirectPosition(new double[]{left, bottom}));
          positions.add(this.positionFactory.createDirectPosition(new double[]{left, top}));
          positions.add(this.positionFactory.createDirectPosition(new double[]{right, top}));
          positions.add(this.positionFactory.createDirectPosition(new double[]{right, bottom}));
          positions.add(this.positionFactory.createDirectPosition(new double[]{left, bottom}));
...
//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
    ...
    */
    ...
    public void setContent(MapContent context) {
              ...
              public void eventDispatched(AWTEvent event) {
                switch(event.getID()){
                  case ExtendedMapContentEvent.LAYER_SELECTABLILITY_CHANGED:
                    Common.logger.log(Level.FINE, "Selectable Layers changed: "+((ExtendedMapContent)event.getSource()).getSelectableLayers());
                  ...
                  case ExtendedMapContentEvent.LAYER_FOCUSABILITY_CHANGED:
                    Common.logger.log(Level.FINE, "Focusable Layers changed: "+((ExtendedMapContent)event.getSource()).getFocusableLayers());
        ...
        Common.logger.log(Level.INFO, "Created geometry builder with CRS:"+getCoordinateReferenceSystem());
        try {
          this.positionFactory   = builder.getPositionFactory();
        } catch (Exception e) {
          Common.logger.log(Level.SEVERE, "Cannot determine position factory: "+e.getMessage(), e);
          try {
            this.positionFactory = GeometryFactoryFinder.getPositionFactory( new Hints( Hints.CRS, DefaultGeographicCRS.WGS84 ) );
            Common.logger.log(Level.WARNING, "Load default position factory with WSG84 CRS");
          } catch (FactoryRegistryException e1) {
            Common.logger.log(Level.SEVERE, "Cannot determine default position factory: "+e.getMessage(), e);
            Common.logger.log(Level.SEVERE, "Maybe gt-jts-wrapper jar is not in your classpath.");
            this.positionFactory = null;
          }
        }
        
        try {
          this.primitiveFactory  = builder.getPrimitiveFactory();
        } catch (Exception e) {
   ...
   */
  public Position getTerrainCoordinates(int x, int y){
    ...
    if (this.mapArea != null){
      ...
      mapPoint = this.positionFactory.createDirectPosition(new double[]{((x * width) / (double) bounds.width) + this.mapArea.getMinX(),
              (((bounds.getHeight() - y) * height) / (double) bounds.height)
              + this.mapArea.getMinY()});

      bounds = null;
    } else {
      mapPoint = this.positionFactory.createDirectPosition(new double[]{x, y});
    }
